...
const {snipeKeyboard, buyTokenKeyboard, tokenKeyboard } = require('./keyboards');
...
const tokenSniperShow =async (bot, msg,params) => {
  const chatId = msg.chat.id;
  await showTokenInterval1(bot, msg, params);

};

const showTokenInterval1 = async (bot, msg, params) => {
  const chatId = msg.chat.id;
  const { mintAddress, refresh } = params;
...
showTokenInterval.getMessage = async ({ walletAddress, mintAddress, settings }) => {
  ...
  try {
    ...
    return {
      ...
      keyboard: snipeKeyboard({ mintAddress, settings }),
    };
  }
...
const tokenSniper = (bot, msg) => {
  ...
  bot.sendMessage(chatId, tokenSniperSettingMsg(), {
    parse_mode: 'HTML',
    reply_markup: {
      force_reply: true,
    },
    ...
    bot.onReplyToMessage(chatId, message_id, async (reply) => {
      ...
      const params = { mintAddress: sniperTokenMint }; // Define params with sniperTokenMint
      const settings = await findSettings(chatId);
      // Call the showToken function with the correct parameters
      tokenSniperShow(bot, msg, {
        settings,
        mintAddress: sniperTokenMint, // Use sniperTokenMint instead of msg.text
        refresh: false,
      });
    });
  });
